...
/**
 * A span is just the difference between two checkpoints. While the closing one
 * is still missing it counts against `now`, which is why it keeps ticking while
 * the steps inside it come and go.
 */
function elapsedForSpan(span, now) {
  const start = run.captured[getIndex(span.from)];
  if (start === undefined) {
    return null;
  }
  return (run.captured[getIndex(span.to)] ?? now) - start;
}

const isSpanRunning = (span) => run.captured[getIndex(span.to)] === undefined;

function collectSpans(now) {
  return SPANS.map((span) => ({ type: span.type, elapsedMs: elapsedForSpan(span, now) })).filter(
    (span) => span.elapsedMs !== null,
  );
}
...
/** One row per result time, built once and updated in place. */
const resultRows = SPANS.map((span) => {
  const item = document.createElement('li');
  ...
  dom.resultList.append(item);

  return { span, item, title, value };
});
...
function archiveRun() {
  ...
  const entry = {
    ...
    spans: collectSpans(run.captured[run.captured.length - 1]),
  };
...
function buildRunReport(entry) {
  ...
  if (entry.spans?.length) {
    ...
    for (const span of entry.spans) {
      lines.push(`${spanName(span.type)}\t${formatTenths(span.elapsedMs)}`);
    }
  }
...
function renderResults(now) {
  ...
  for (const row of resultRows) {
    const elapsed = elapsedForSpan(row.span, now);
    row.item.hidden = elapsed === null;
    ...
    anyVisible = true;
    row.title.textContent = spanName(row.span.type);
    row.value.textContent = formatTenths(elapsed);
    row.item.classList.toggle('is-running', isSpanRunning(row.span));
  }

  dom.resultsCard.hidden = !anyVisible;
}
...
function renderHistory() {
  ...
  for (const entry of history) {
    ...
    for (const span of entry.spans ?? []) {
      const item = document.createElement('li');
      item.className = 'run-step run-step--result';
      const name = document.createElement('span');
      name.textContent = spanName(span.type);
      const value = document.createElement('span');
      value.textContent = formatTenths(span.elapsedMs);
      item.append(name, value);
      steps.append(item);
    }
